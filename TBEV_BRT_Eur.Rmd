---
title: "Boosted regression tree approach in R: West Nile risk mapping using Padi-web
  and ECDC data"
output:
...
1. Build a database of WNV presence/absence in Europe (based on Padiweb and ECDC data) and its related climatic covariates
2. Account for spatial autocorrelation by involving spatial cross-validation methods
3. Run the BRT approach to generate an ecological suitability model
4. Create an ecological suitability map based on the previous model
5. Assess the relative influence of each variable and the response curves
...
#### Loading WNV cases following the ECDC and Padiweb datasets. 

Since ECDC data is reported at the TBEV level, this will be our study unit
...
!!!!!!!Attention!!!!!
Unknown subtype was used to make the pseudo-absence, fortunately only 3 administrative polygons have only a presence of unkown subtype, so it should not affect the model too much. But keep in mind to change it.
...
#map of the presence of TBEV-Eur
```{r}
# Function to plot a single map
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/Eur_pres.png", width = 3200, height = 1600, res = 200)  # 2400x2400 pixels at 300 PPI
...
plot(presence_data_Eur, col = presence_fill, border = NA, lwd = 0.1, add = TRUE)
...
# Use inset to position the legend within the plotting area
legend("bottomright", legend = "TBEV-Eur occurence detected",
       pch = 15, col = presence_fill, pt.cex = 6,cex= 1.5 ,x.intersp = 2, bty = "n", xpd = TRUE, inset = c(0.05, 0.9))
```



colourScale = c("#E0E0E0","#FF3333")
cols = colourScale[(WNVdata$Eur)+1]
plot(contour, lwd=0.4, border="gray30", col=NA); plot(TBEV, col=cols, lwd=0.1, add=T)
title("ECDC")

cols = colourScale[(WNVdata$Sib)+1]
plot(contour, lwd=0.4, border="gray30", col=NA); plot(TBEV, col=cols, lwd=0.1, add=T)
title("Padiweb")

cols = colourScale[(WNVdata$FE)+1]
plot(contour, lwd=0.4, border="gray30", col=NA); plot(TBEV, col=cols, lwd=0.1, add=T)
title("Both")

cols = colourScale[(WNVdata$Bal)+1]
plot(contour, lwd=0.4, border="gray30", col=NA); plot(TBEV, col=cols, lwd=0.1, add=T)
title("Padiweb")

cols = colourScale[(WNVdata$unknown)+1]
plot(contour, lwd=0.4, border="gray30", col=NA); plot(TBEV, col=cols, lwd=0.1, add=T)
title("Both")
...
<div class="alert alert-info">
  <strong>Question</strong> What differences do you spot between ECDC and Padiweb datasets?
</div>
...
for(i in 1:nrow(TBEV.data)){
  ...
  data_Eur[i,"temperature_winter"] = exact_extract(mean(temp_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_Eur[i,"temperature_spring"] = exact_extract(mean(temp_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_Eur[i,"temperature_summer"] = exact_extract(mean(temp_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_Eur[i,"temperature_autumn"] = exact_extract(mean(temp_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  
  data_Eur[i,"precipitation_winter"] = exact_extract(mean(precp_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_Eur[i,"precipitation_spring"] = exact_extract(mean(precp_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_Eur[i,"precipitation_summer"] = exact_extract(mean(precp_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_Eur[i,"precipitation_autumn"] = exact_extract(mean(precp_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  
  data_Eur[i,"relative_humidity_winter"] = exact_extract(mean(rel_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"relative_humidity_spring"] = exact_extract(mean(rel_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"relative_humidity_summer"] = exact_extract(mean(rel_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"relative_humidity_autumn"] = exact_extract(mean(rel_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  
  data_Eur[i,"croplands"] = exact_extract(mean(croplands_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"pastures"] = exact_extract(mean(pastures_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"urbanAreas"] = exact_extract(mean(urbanAreas_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"primaryForest"] = exact_extract(mean(primaryForest_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"secondaryForest"] = exact_extract(mean(secondaryForest_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_Eur[i,"rangelands"] = exact_extract(mean(rangelands_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  
  data_Eur[i,"population"] = exact_extract(mean(pLog),TBEV[(which(TBEV@data$ID == id)),],fun="sum")
  
  data_Eur[i,"longitude"] = coordinates(TBEV[(which(TBEV@data$ID == id)),])[,1]
  data_Eur[i,"latitude"] = coordinates(TBEV[(which(TBEV@data$ID == id)),])[,2]
  data_Eur[i,"ID"] = id
  data_Eur[i,"year"] = year
  data_Eur[i,"response"] = resp
}
...
<div class="alert alert-info">
  <strong>Question</strong> What is the study unit?
</div>
...
#Eur
correlogram = ncf::correlog(data_Eur[,"longitude"], data_Eur[,"latitude"], data_Eur[,"response"], na.rm=T, increment=10, resamp=0, latlon=T)
...
## inputs: dataframe used for brt, brt_model (brt_model_outputs_Eur3)
# 1. create an empty matrix to fill with covariates
envVariableNames = names(newdata_Eur)
...
# 2. create a matrix to fill with the relative influence of each covariate
relativeInfluences_Eur = matrix(0, nrow=length(envVariableNames), ncol=length(brt_model_outputs_Eur))
row.names(relativeInfluences_Eur) = envVariableNames
for (i in 1:100) {
  for (k in 1:length(envVariableNames))
  {
    relativeInfluences_Eur[k,i] = summary(brt_model_outputs_Eur[[i]])[gsub("-","\\.",envVariableNames)[k],"rel.inf"]
  }
}

relativeInfluences_Eur_mean <- matrix(rowMeans(relativeInfluences_Eur), nrow=length(envVariableNames), ncol=1)
row.names(relativeInfluences_Eur_mean) = envVariableNames

ci_lower_Eur <- c()
ci_upper_Eur <- c()
for (i in 1:length(envVariableNames)) {
  # Calculate 95% confidence interval
  ci <- t.test(relativeInfluences_Eur[i, ])$conf.int
  # Append results to lists
  ci_lower_Eur <- c(ci_lower_Eur, ci[1])
  ci_upper_Eur <- c(ci_upper_Eur, ci[2])
}

ci_envVariableNames_Eur <- data.frame(envVariableNames, round(ci_lower_Eur,2), round(ci_upper_Eur,2))
mean_relativeInfluences_Eur <- data.frame(round(relativeInfluences_Eur_mean,2))

#save it
#write.table(round(relativeInfluences_Eur_mean,1),paste0("Relative_influences_Eur.csv"), quote=F, sep=",")



# 2. plot presence curves
data_curv_Eur = data_Eur[which(data_Eur[,"response"]==1),] #assuming presence is in column "presence"
...
{
  minV = min(data_curv_Eur[,envVariableNames[j]], na.rm=T)
  maxV = max(data_curv_Eur[,envVariableNames[j]], na.rm=T)
  medianV = median(data_curv_Eur[,envVariableNames[j]], na.rm=T)
...
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/100BRT_RI_parameters_Eur2.png", width=2400, height=2400, res=300)
...
for (i in 1:length(envVariableNames))
{
  ...
  brt_model_outputs_Eur = brt_model_outputs_Eur
  
  for (j in 1:length(brt_model_outputs_Eur))
  {
    n.trees = brt_model_outputs_Eur[j]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    prediction = predict.gbm(brt_model_outputs_Eur[[j]], newdata=df, n.trees, type, single.tree)
    if ((j == 1))
    {
  ...
  cols = "red"
    for (l in 1:length(brt_model_outputs_Eur))
    {
      if ((l == 1))
      {
        plot(dfs[,envVariableNames[i]],predictions_list[[l]],col=cols,ann=F,axes=F,lwd=0.2,type="l",xlim=c(minX,maxX),ylim=c(minY,maxY))
        mtext(paste0(round(relativeInfluences_Eur_mean[i, 1], 1), "%"), side = 3, line = 0.5, cex = 0.8, col = "black")
...
#### 4. Create an ecological suitability map for present time


```{r}
nberOfReplicates = 100 #you have 100 BRT models
prediction_present_Eur = matrix(0, nrow=length(pop), ncol=nberOfReplicates)

for(i in 1:nberOfReplicates){
    n.trees = brt_model_outputs_Eur[[i]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    object = brt_model_outputs_Eur[[i]]; prediction = predict.gbm(object, newdata_all, n.trees, type, single.tree)
    prediction_present_Eur[,i] = prediction
}

prediction_present_Eur_mean <- rowMeans(prediction_present_Eur)
```

```{r results1, echo=, message=FALSE, warning=FALSE}

#Eur
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_Eur_present.png", width = 3200, height = 1600, res = 200)  # 2400x2400 pixels at 300 PPI
# Set up plotting layout
par(mfrow = c(1, 1),  #  rows, columns layout
    oma = c(0, 0, 0, 0),  # Outer margins
    mar = c(0, 0, 0, 0) + 0.1,  # Reduce plot margins to fit better
...
plot_map(i, key, prediction_present_Eur_mean)
...
TBEV@data$pred_Eur = prediction_present_Eur_mean

TBEV_sorensen_Eur <- TBEV@data

#One line per year
TBEV_sorensen_Eur <- TBEV_sorensen_Eur %>%
  filter(sum > 0) 

#Delete all NA's
TBEV_sorensen_Eur <- TBEV_sorensen_Eur %>% filter(year != "NA")

#ignore years after 2019
TBEV_sorensen_Eur <- TBEV_sorensen_Eur %>% filter(year < 2020)


data_sorensen_Eur = data.frame("response" = TBEV_sorensen_Eur$Eur, "ecological suitability" = TBEV_sorensen_Eur$pred_Eur) #column 1: presence/absence, column 2: ecological suitability
thres = rep(1:100,each = 1)*0.01
data_comp_Eur = as.data.frame(matrix(0,nrow = length(thres), ncol = 6))
colnames(data_comp_Eur) = c("TP","FP","FN","TN","SI","threshold")
data_comp_Eur[,"threshold"] = thres
for(i in 1:length(thres)){
    for(j in 1: nrow(data_sorensen_Eur)){
      # True positive
      if(data_sorensen_Eur[j,1] == 1 && data_sorensen_Eur[j,2] >= thres[i]){
        data_comp_Eur[i,"TP"] = data_comp_Eur[i,"TP"] + 1
      }
      # False negative
      if(data_sorensen_Eur[j,1] == 1 && data_sorensen_Eur[j,2] < thres[i]){
        data_comp_Eur[i,"FN"] = data_comp_Eur[i,"FN"] + 1
      }
      if(data_sorensen_Eur[j,1] == 0 && data_sorensen_Eur[j,2] >= thres[i]){
        data_comp_Eur[i,"FP"] = data_comp_Eur[i,"FP"] + 1
      }
      if(data_sorensen_Eur[j,1] == 0 && data_sorensen_Eur[j,2] < thres[i]){
        data_comp_Eur[i,"TN"] = data_comp_Eur[i,"TN"] + 1
      }
    }  
}

x = sum(data_sorensen_Eur$response == 1)/sum(data_sorensen_Eur$response == 0)*(1 -sum(data_sorensen_Eur$response == 1)/nrow(data_sorensen_Eur))/(sum(data_sorensen_Eur$response == 1)/nrow(data_sorensen_Eur))

data_comp_Eur[,"SI"] = (2*(data_comp_Eur[,"TP"]))/((2*data_comp_Eur[,"TP"])+(x*data_comp_Eur[,"FP"])+data_comp_Eur[,"FN"])  
...
for (i in 1:length(folder_model_past)){
  ...
  for(k in 1:nberOfReplicates){
    n.trees = brt_model_outputs_Eur[[k]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    object = brt_model_outputs_Eur[[k]]; prediction = predict.gbm(object, newdata, n.trees, type, single.tree)
    prediction_tot[,k] = prediction
  }
  key <- paste0("prediction_past_", folder_model_past[i], "_",year_intervals[j])
  prediction_past_Eur[[key]] <- prediction_tot
  key <- paste0("data_past_", folder_model_past[i], "_",year_intervals[j])
  data_past_Eur[[key]] <- newdata
...
for (i in 1:length(prediction_past_Eur)){
  key <- names(prediction_past_Eur)[i]
  prediction_past_Eur_mean <- matrix(rowMeans(prediction_past_Eur[[key]]), nrow=length(pop), ncol=1)
  prediction_past_Eur[[key]] <- prediction_past_Eur_mean
}
...
```{r}
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_Eur_past2.png", width = 1600, height = 1600, res = 200) # Adjust width and height to fit 2x6     width = 9600, height = 3200, res = 200)  # Adjust width and height to fit 2x6 layout
...
for (i in 1:length(prediction_past_Eur)) {  # Adjust the range as necessary
  if (i %in% c(6, 12)) {
    key <- names(prediction_past_Eur)[i]
    data <- prediction_past_Eur[[key]]
    plot_map(i, key, data)
...
```{r include=FALSE}
###FUTURE
data_future_Eur <- list()
prediction_future_Eur <- list()
...
for(i in 1:length(models)){
  ...
  # mean per season and conversion to Celsius
      ...
      for(l in 1:nberOfReplicates){
        n.trees = brt_model_outputs_Eur[[l]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
        object = brt_model_outputs_Eur[[l]]; prediction = predict.gbm(object, newdata, n.trees, type, single.tree)
        prediction_tot[,l] = prediction
      }
      prediction_tot = rowMeans(prediction_tot)
      key <- paste(scenario_key, year_key,  sep = "_")
      data_future_Eur[[key]][[model_key]] <- newdata
      prediction_future_Eur[[key]][[model_key]] <- prediction_tot
    }
  }
}

save.image("~/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Data_BRT_Eur.RData")
```
...
```{r}
prediction_future_Eur_mean <- prediction_future_Eur
#take mean of the models predictions
for (key in names(prediction_future_Eur_mean)) {
  model_predictions <- prediction_future_Eur_mean[[key]]
  ...
  prediction_future_Eur_mean[[key]] <- mean_predictions
}
...
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_Eur_future.png", width = 6400, height = 2400, res = 200)  # 2400x2400 pixels at 300 PPI
...
# Loop to generate the plots
for (i in 1:12) {
  key <- names(prediction_future_Eur_mean)[i]
  data <- prediction_future_Eur_mean[[key]]
  plot_map(i, key, data)
...
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_Eur_future2.png", width = 3200, height = 2400, res = 200)  # 2400x2400 pixels at 300 PPI
...
# Loop to generate the plots
for (i in 1:length(prediction_future_Eur_mean)) {  # Adjust the range as necessary
...
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/diff_map.png", width = 4800, height = 2400, res = 200)  # 2400x2400 pixels at 300 PPI


# Définir les comparaisons
comparisons <- list(
  # dif 1 : prediction_past_Eur[[12]] - predictio_past_Eur[[7]]
  list(prediction_past_Eur[[12]], prediction_past_Eur[[7]]),
  list(prediction_past_Sib[[12]], prediction_past_Sib[[7]]),
  list(prediction_past_FE[[12]], prediction_past_FE[[7]]),
  
  # dif 2 : prediction_past_Eur[[12]] - predictio_past_Eur[[6]]
  list(prediction_past_Eur[[12]], prediction_past_Eur[[6]]),
  list(prediction_past_Sib[[12]], prediction_past_Sib[[6]]),
  list(prediction_past_FE[[12]], prediction_past_FE[[6]]),
  
  # dif 3 : prediction_future_Eur_mean[[12]]- prediction_past_Eur[[12]] 
  list(prediction_future_Eur_mean[[12]], prediction_past_Eur[[12]]),
  list(prediction_future_Sib_mean[[12]], prediction_past_Sib[[12]]),
  list(prediction_future_FE_mean[[12]], prediction_past_FE[[12]])
)

# Calculez les différences pour toutes les cartes et trouvez les valeurs minimales et maximales
all_differences <- c()
for (comp in comparisons) {
  future_values <- round(comp[[1]], 2)
  present_values <- round(comp[[2]], 2)
  differences <- round(future_values - present_values, 2)
  significant_differences <- ifelse(differences >= -0.03 & differences <= 0.03, 0, differences)
  all_differences <- c(all_differences, significant_differences)
}

# Définir la plage souhaitée pour l'échelle des couleurs en utilisant les valeurs calculées
min_diff <- min(all_differences, na.rm = TRUE)
max_diff <- max(all_differences, na.rm = TRUE)

# Assurez-vous que la plage est de -0.7 à 0.7 même si les différences réelles sont plus petites
min_diff <- -0.7
max_diff <- 0.7

# Définir une échelle de couleurs personnalisée avec le blanc au point médian
custom_palette <- colorRampPalette(c("darkgreen", "white", "darkred"))

# Créer une séquence de couleurs avec le blanc au centre
n_colors <- 121
colors <- custom_palette(n_colors)

# Créer des ruptures pour l'échelle de couleurs, en veillant à ce que zéro soit au centre
breaks <- c(seq(min_diff, 0, length.out = n_colors / 2 + 1),
            seq(0, max_diff, length.out = n_colors / 2 + 1)[-1])
...
# Fonction pour mapper les différences à l'échelle de couleurs
map_to_color <- function(value, breaks, colors) {
  idx <- findInterval(value, breaks, all.inside = TRUE)
  colors[idx]
}

# Configuration de la mise en page pour les graphiques
par(mfrow = c(3, 3),  # disposition en lignes et colonnes
    oma = c(0, 0, 0, 0),  # marges extérieures
    mar = c(0, 0, 2, 0) + 0.1,  # marges des graphiques réduites
    mgp = c(0, 0, 0),  # ligne de marge pour le titre, les étiquettes des axes et la ligne de l'axe
    cex = 0.8)  # réduire la taille du texte pour mieux s'adapter

# Boucle pour chaque carte à tracer
for (i in 1:length(comparisons)) {
  comp <- comparisons[[i]]
  
  # Créer un nouveau dataframe pour stocker les différences entre les valeurs de la pertinence écologique
  data_diff <- data.frame(matrix(ncol = 3, nrow = 3924))
  data_diff[,1] <- round(comp[[1]], 2) # projections moyennes des valeurs de la pertinence écologique 1
  data_diff[,2] <- round(comp[[2]], 2) # projections moyennes des valeurs de la pertinence écologique 2
  data_diff[,3] <- round(data_diff[,1] - data_diff[,2], 2) # différence entre les valeurs de projection 1 - valeurs 2
  names(data_diff) <- c("Future","Present", "difference")
  
  # Définir les différences insignifiantes à 0
  data_diff$map_differences <- ifelse(data_diff$difference >= -0.03 & data_diff$difference <= 0.03, 0, data_diff$difference)
  
  # Appliquer la normalisation pour mapper les différences
  normalized_diff <- normalize(data_diff$map_differences, min_diff, max_diff)
  
  # Appliquer la fonction aux différences
  cols_diff <- sapply(data_diff$map_differences, map_to_color, breaks, colors)
  
  plot(contour, lwd = 0.4, border = "gray30", col = NA)
  plot(TBEV.projection, col = cols_diff, border = "gray80", lwd = 0.05, add = TRUE)
  
  # Ajouter les titres et annotations
  if(i == 1){ title(main = "TBEV-Eur", cex.main = 2, col.main = "black") }
  if(i == 2){ title(main = "TBEV-Sib", cex.main = 2, col.main = "black") }
  if(i == 3){ title(main = "TBEV-FE", cex.main = 2, col.main = "black") }
  
  if(i==1){text(x = par("usr")[1] + (par("usr")[2] - par("usr")[1]) * 0.05, y = par("usr")[4] - (par("usr")[4] - par("usr")[3]) * 0.05, labels = "a", cex = 2.5, col = "black", adj = 0, font = 2)}
  if(i==4){text(x = par("usr")[1] + (par("usr")[2] - par("usr")[1]) * 0.05, y = par("usr")[4] - (par("usr")[4] - par("usr")[3]) * 0.05, labels = "b", cex = 2.5, col = "black", adj = 0, font = 2)}
  if(i==7){text(x = par("usr")[1] + (par("usr")[2] - par("usr")[1]) * 0.05, y = par("usr")[4] - (par("usr")[4] - par("usr")[3]) * 0.05, labels = "c", cex = 2.5, col = "black", adj = 0, font = 2)}
  

}


# Créer un raster de légende avec les ruptures et les couleurs définies
legend_rast <- raster(matrix(seq(min_diff, max_diff, length.out = n_colors), ncol = 1))
numbers <- c(-0.6,-0.3,0,0.3,0.6)  # Specify the positions of the labels
plot(legend_rast, legend.only = TRUE, add = TRUE, col = colors, 
     legend.width = 0.5, legend.shrink = 2, 
     smallplot = c(0.9, 0.91, 0.2, 0.5), adj = 3, 
     axis.args = list(
       at = numbers, # Spécifiez les positions des étiquettes
       labels = round(numbers, 2), # Spécifiez les étiquettes correspondantes
       cex.axis = 1.5,      # Ajustez cette valeur pour la taille des chiffres
       lwd = 1,             # Largeur des lignes de l'axe
       col = "gray30", 
       lwd.tick = 1,      # Largeur des petits traits des graduations
