...
#map of the presence of TBEV-FE
```{r}
# Function to plot a single map
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/FE_pres.png", width = 3200, height = 1600, res = 200)  # 2400x2400 pixels at 300 PPI
...
plot(presence_data_FE, col = presence_fill, border = NA, lwd = 0.1, add = TRUE)
...
# Use inset to position the legend within the plotting area
legend("bottomright", legend = "TBEV-FE occurence detected",
...
for (i in 1:nrow(TBEV.data)){
  ...
  data_FE[i,"temperature_winter"] = exact_extract(mean(temp_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_FE[i,"temperature_spring"] = exact_extract(mean(temp_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_FE[i,"temperature_summer"] = exact_extract(mean(temp_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  data_FE[i,"temperature_autumn"] = exact_extract(mean(temp_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")-273.15 # conversion to Celsius degrees
  
  data_FE[i,"precipitation_winter"] = exact_extract(mean(precp_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_FE[i,"precipitation_spring"] = exact_extract(mean(precp_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_FE[i,"precipitation_summer"] = exact_extract(mean(precp_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  data_FE[i,"precipitation_autumn"] = exact_extract(mean(precp_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")*60*60*24 # conversion to kg/m2/day
  
  data_FE[i,"relative_humidity_winter"] = exact_extract(mean(rel_winter),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"relative_humidity_spring"] = exact_extract(mean(rel_spring),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"relative_humidity_summer"] = exact_extract(mean(rel_summer),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"relative_humidity_autumn"] = exact_extract(mean(rel_autumn),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  
  data_FE[i,"croplands"] = exact_extract(mean(croplands_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"pastures"] = exact_extract(mean(pastures_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"urbanAreas"] = exact_extract(mean(urbanAreas_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"primaryForest"] = exact_extract(mean(primaryForest_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"secondaryForest"] = exact_extract(mean(secondaryForest_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  data_FE[i,"rangelands"] = exact_extract(mean(rangelands_temp),TBEV[(which(TBEV@data$ID == id)),],fun="mean")
  
  data_FE[i,"population"] = exact_extract(mean(pLog),TBEV[(which(TBEV@data$ID == id)),],fun="sum")
  
  data_FE[i,"longitude"] = coordinates(TBEV[(which(TBEV@data$ID == id)),])[,1]
  data_FE[i,"latitude"] = coordinates(TBEV[(which(TBEV@data$ID == id)),])[,2]
  data_FE[i,"ID"] = id
  data_FE[i,"year"] = year
  data_FE[i,"response"] = resp
}
...
save.image(file = "/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Data_BRT_FE.RData")
...
#FE
correlogram = ncf::correlog(data_FE[,"longitude"], data_FE[,"latitude"], data_FE[,"response"]==1, na.rm=T, increment=10, resamp=0, latlon=T)
...
newdata_FE = data_FE[,c(6:24)]
data_FE = na.omit(data_FE)


n.folds = 5

theRanges_FE = c(2000,2000)*1000 # distance has to be in in meters
gbm.x = 6:24
gbm.y = 5
offset = NULL
...
site.weights_FE = rep(1, dim(data_FE)[1])
...
AUCs_FE = matrix(nrow=1, ncol=1)
SSBs_FE = matrix(nrow=1, ncol=1)
colnames(AUCs_FE) = c("SCV3")
colnames(SSBs_FE) = c("SCV3")
...
brt_model_outputs_FE <- list()
auc_values_FE <- numeric(100)

for (i in 1:100) {
  repeat {
    folds_with_similar_sizes <- FALSE
    while (!folds_with_similar_sizes) {
      spdf_FE = SpatialPointsDataFrame(data_FE[c("longitude", "latitude")], data_FE, proj4string = crs(TBEV))
      myblocks_FE = cv_spatial(spdf_FE, column = "response", k = n.folds, size = theRanges_FE, selection = "random", progress = FALSE)
      
      fold.vector_FE = myblocks_FE$folds_ids
      fold.vector_presences_FE = fold.vector_FE[which(data_FE[, 5] == 1)]
      counts = hist(fold.vector_presences_FE, plot = FALSE)$counts
      props = counts[which(counts > 0)] / sum(counts)
      print(round(props, 2))
      if (min(props) > 0.05) folds_with_similar_sizes <- TRUE
    }
    
    brt_model_FE = gbm.step(data_FE, gbm.x, gbm.y, offset, fold.vector_FE, tree.complexity, learning.rate, bag.fraction, site.weights_FE,
  ...
  brt_model_outputs_FE[[i]] <- brt_model_FE
  auc_values_FE[i] <- brt_model_FE$cv.statistics$discrimination.mean
}

# Calculate 95% confidence interval
ci_AUC_FE <- t.test(auc_values_FE)$conf.int
...
## inputs: dataframe used for brt, brt_model (brt_model_outputs_FE3)
# 1. create an empty matrix to fill with covariates
envVariableNames = names(newdata_FE)
...
relativeInfluences_FE = matrix(0, nrow=length(envVariableNames), ncol=length(brt_model_outputs_FE))
row.names(relativeInfluences_FE) = envVariableNames
for (i in 1:100) {
  for (k in 1:length(envVariableNames))
  {
    relativeInfluences_FE[k,i] = summary(brt_model_outputs_FE[[i]])[gsub("-","\\.",envVariableNames)[k],"rel.inf"]
  }
}

relativeInfluences_FE_mean <- matrix(rowMeans(relativeInfluences_FE), nrow=length(envVariableNames), ncol=1)
row.names(relativeInfluences_FE_mean) = envVariableNames

ci_lower_FE <- c()
ci_upper_FE <- c()
for (i in 1:length(envVariableNames)) {
  # Calculate 95% confidence interval
  ci <- t.test(relativeInfluences_FE[i, ])$conf.int
  # Append results to lists
  ci_lower_FE <- c(ci_lower_FE, ci[1])
  ci_upper_FE <- c(ci_upper_FE, ci[2])
}

ci_envVariableNames_FE <- data.frame(envVariableNames, round(ci_lower_FE,2), round(ci_upper_FE,2))
mean_relativeInfluences_FE <- data.frame(round(relativeInfluences_FE_mean,2))



#save it
write.table(round(relativeInfluences_FE,1),paste0("Relative_influences_FE.csv"), quote=F, sep=",")
# 2. plot presence curves
data_curv_FE = data_FE[which(data_FE[,"response"]==1),] #assuming presence is in column "presence"
...
{
  minV = min(data_curv_FE[,envVariableNames[j]], na.rm=T)
  maxV = max(data_curv_FE[,envVariableNames[j]], na.rm=T)
  medianV = median(data_curv_FE[,envVariableNames[j]], na.rm=T)
...
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/100BRT_RI_parameters_FE2.png", width=2400, height=2400, res=300)
...
for (i in 1:length(envVariableNames))
{
  ...
  brt_model_outputs_FE = brt_model_outputs_FE
  
  for (j in 1:length(brt_model_outputs_FE))
  {
    n.trees = brt_model_outputs_FE[j]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    prediction = predict.gbm(brt_model_outputs_FE[[j]], newdata=df, n.trees, type, single.tree)
    if ((j == 1))
    {
  ...
  cols = "red"
    for (l in 1:length(brt_model_outputs_FE))
    {
      if (( l == 1)) 
      {
        plot(dfs[,envVariableNames[i]],predictions_list[[l]],col=cols,ann=F,axes=F,lwd=0.2,type="l",xlim=c(minX,maxX),ylim=c(minY,maxY))
        mtext(paste0(round(relativeInfluences_FE_mean[i, 1], 1), "%"), side = 3, line = 0.5, cex = 0.8, col = "black")
  ...
  # plot instructions
  axis(side=1, lwd.tick=0.2, cex.axis=0.7, lwd=0, tck=-0.030, col.axis="gray30", mgp=c(0,0,0))
...
prediction_present_FE = matrix(0, nrow=length(pop), ncol=nberOfReplicates)

for(i in 1:nberOfReplicates){
    n.trees = brt_model_outputs_FE[[i]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    object = brt_model_outputs_FE[[i]]; prediction = predict.gbm(object, newdata_all, n.trees, type, single.tree)
    prediction_present_FE[,i] = prediction
}

prediction_present_FE_mean <- rowMeans(prediction_present_FE)
...
#FE
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_FE_present.png", width = 3200, height = 1600, res = 200)  # 2400x2400 pixels at 300 PPI
# Set up plotting layout
par(mfrow = c(1, 1),  #  rows, columns layout
    oma = c(0, 0, 0, 0),  # Outer margins
    mar = c(0, 0, 0, 0) + 0.1,  # Reduce plot margins to fit better
...
plot_map(i, key, prediction_present_FE_mean)
...
```{r}
TBEV@data$pred_FE = prediction_present_FE_mean

TBEV_sorensen_FE <- TBEV@data

#One line per year
TBEV_sorensen_FE <- TBEV_sorensen_FE %>%
  filter(sum > 0) 

#Delete all NA's
TBEV_sorensen_FE <- TBEV_sorensen_FE %>% filter(year != "NA")

#ignore years after 2019
TBEV_sorensen_FE <- TBEV_sorensen_FE %>% filter(year < 2020)


data_sorensen_FE = data.frame("response" = TBEV_sorensen_FE$FE, "ecological suitability" = TBEV_sorensen_FE$pred_FE) #column 1: presence/absence, column 2: ecological suitability
thres = rep(1:100,each = 1)*0.01
data_comp_FE = as.data.frame(matrix(0,nrow = length(thres), ncol = 6))
colnames(data_comp_FE) = c("TP","FP","FN","TN","SI","threshold")
data_comp_FE[,"threshold"] = thres
for(i in 1:length(thres)){
    for(j in 1: nrow(data_sorensen_FE)){
      # True positive
      if(data_sorensen_FE[j,1] == 1 && data_sorensen_FE[j,2] >= thres[i]){
        data_comp_FE[i,"TP"] = data_comp_FE[i,"TP"] + 1
      }
      # False negative
      if(data_sorensen_FE[j,1] == 1 && data_sorensen_FE[j,2] < thres[i]){
        data_comp_FE[i,"FN"] = data_comp_FE[i,"FN"] + 1
      }
      if(data_sorensen_FE[j,1] == 0 && data_sorensen_FE[j,2] >= thres[i]){
        data_comp_FE[i,"FP"] = data_comp_FE[i,"FP"] + 1
      }
      if(data_sorensen_FE[j,1] == 0 && data_sorensen_FE[j,2] < thres[i]){
        data_comp_FE[i,"TN"] = data_comp_FE[i,"TN"] + 1
      }
    }  
}

x = sum(data_sorensen_FE$response == 1)/sum(data_sorensen_FE$response == 0)*(1 -sum(data_sorensen_FE$response == 1)/nrow(data_sorensen_FE))/(sum(data_sorensen_FE$response == 1)/nrow(data_sorensen_FE))

data_comp_FE[,"SI"] = (2*(data_comp_FE[,"TP"]))/((2*data_comp_FE[,"TP"])+(x*data_comp_FE[,"FP"])+data_comp_FE[,"FN"])
...
for (i in 1:length(folder_model_past)){
  ...
  for(k in 1:nberOfReplicates){
    n.trees = brt_model_outputs_FE[[k]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
    object = brt_model_outputs_FE[[k]]; prediction = predict.gbm(object, newdata, n.trees, type, single.tree)
    prediction_tot[,k] = prediction
  }
  key <- paste0("prediction_past_", folder_model_past[i], "_",year_intervals[j])
  prediction_past_FE[[key]] <- prediction_tot
  key <- paste0("data_past_", folder_model_past[i], "_",year_intervals[j])
  data_past_FE[[key]] <- newdata
...
for (i in 1:length(prediction_past_FE)){
  key <- names(prediction_past_FE)[i]
  prediction_past_FE_mean <- matrix(rowMeans(prediction_past_FE[[key]]), nrow=length(pop), ncol=1)
  prediction_past_FE[[key]] <- prediction_past_FE_mean
}
...
```{r}
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_FE_past2.png", width = 1600, height = 1600, res = 200) # Adjust width and height to fit 2x6     width = 9600, height = 3200, res = 200)  # Adjust width and height to fit 2x6 layout
...
for (i in 1:length(prediction_past_FE)) {  # Adjust the range as necessary
  if (i %in% c(6, 12)) {
    key <- names(prediction_past_FE)[i]
    data <- prediction_past_FE[[key]]
    plot_map(i, key, data)
...
for(i in 1:length(models)){
  ...
  # mean per season and conversion to Celsius
      ...
      for(l in 1:nberOfReplicates){
        n.trees = brt_model_outputs_FE[[l]]$gbm.call$best.trees; type = "response"; single.tree = FALSE
        object = brt_model_outputs_FE[[l]]; prediction = predict.gbm(object, newdata, n.trees, type, single.tree)
        prediction_tot[,l] = prediction
      }
      prediction_tot = rowMeans(prediction_tot)
      key <- paste(scenario_key, year_key,  sep = "_")
      data_future_FE[[key]][[model_key]] <- newdata
      prediction_future_FE[[key]][[model_key]] <- prediction_tot
    }
  }
}

save.image("~/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Data_BRT_FE.RData")
```
...
```{r}
prediction_future_FE_mean <- prediction_future_FE
#take mean of the models predictions
for (key in names(prediction_future_FE_mean)) {
  model_predictions <- prediction_future_FE_mean[[key]]
  ...
  prediction_future_FE_mean[[key]] <- mean_predictions
}
...
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_FE_future.png", width = 6400, height = 2400, res = 200)  # 2400x2400 pixels at 300 PPI
...
# Loop to generate the plots
for (i in 1:12) {
  key <- names(prediction_future_FE_mean)[i]
  data <- prediction_future_FE_mean[[key]]
  plot_map(i, key, data)
...
# Start PNG device
png("/Users/loicgonzalez/Library/CloudStorage/OneDrive-UniversitéLibredeBruxelles/Bioingénieur/MA-2- 2023-2024/Mémoire/GitHub/Memoire/Données & script/Ecological_suitability_maps/ecological_suitability_FE_future2.png", width = 3200, height = 2400, res = 200)  # 2400x2400 pixels at 300 PPI
...
# Loop to generate the plots
for (i in 1:length(prediction_future_FE_mean)) {  # Adjust the range as necessary
